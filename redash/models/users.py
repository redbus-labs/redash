...
class User(TimestampMixin, db.Model, BelongsToOrgMixin, UserMixin, PermissionsCheckMixin):
	...
	all_tags_result = set()
        for g1 in Group.query:
		all_tags_result |= set(g1.allowed_tags)
        all_allowed_tags = list(all_tags_result)
	...
	Grps = Group.query.filter(Group.id.in_(self.group_ids))
        for g in Group.query.filter(Group.id.in_(self.group_ids)):
            tags_result |= set(g.allowed_tags)
            for k in g.restricting_parameters:
                restricted_value = g.restricting_parameters[k]
                if "currentUser.email" in restricted_value:
                    restricted_value = [self.email]
                if k.lower() in restricting_params_result:
                    restricting_params_result[k.lower()].extend(restricted_value)
                else:
                    restricting_params_result[k.lower()] = restricted_value
	...
	if tags and list(tags_result & set(tags)) == []:
	    error_message = 'You do not have access to view data. Please contact your administrator'
	    return error_message, parameters		
        #if tags and all_allowed_tags :
        #    tags_to_check = list(set(all_allowed_tags) & set(tags))
	#    logger.info('tag check:{}'.format(tags_to_check))
        #    flag = 0
